...
class EnergyScoreModel(AbstractProcessRewardModel):
    ...
    def score(
        ...
        for start_idx in range(0, len(prompts), batch_size):
            ...
            with torch.no_grad():
                ...
                scores = [[s.item()] for s in self._calculate_energy(outputs.logits)]
    ...
    def _calculate_energy(self, logits):
        with torch.no_grad():
            ...
            energy = -torch.logsumexp(logits, dim=-1)   # (B, L)
            avg_energy = energy.mean(dim=-1)    # (B,)
