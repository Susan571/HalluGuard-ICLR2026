...
class PTrueScoreModel(AbstractProcessRewardModel):
    ...
    def score(
        ...
        for start_idx in range(0, len(prompts), batch_size):
            ...
            with torch.no_grad():
                ...
                scores = [[s.item()] for s in self._calculate_ptrue_score(outputs.logits)]
    ...
    def _calculate_ptrue_score(self, logits):
        with torch.no_grad():
            ...
            max_probs = torch.max(probs, dim=-1)[0]  # (B, L-1)
            
            # P(true) is the average of maximum probabilities
            ptrue_score = torch.mean(max_probs, dim=-1)
