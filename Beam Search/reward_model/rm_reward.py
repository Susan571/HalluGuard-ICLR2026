...
class VllmOutcomeRewardModel(AbstractOutcomeRewardModel):
    def __init__(self, model_name: str, gpu_memory_utilization:float, **kwargs):
...
class VllmProcessRewardModel(AbstractProcessRewardModel):
    def __init__(self, reward_model: str, gpu_memory_utilization=0.3, **kwargs):
        os.environ["VLLM_ALLOW_LONG_MAX_MODEL_LEN"] = "1"
        num_gpus = torch.cuda.device_count()
        print(f"Number of GPUs: {num_gpus}")
        
        self.model = LLM(model=reward_model, 
                    task="reward",
        ...
        if reward_model == "Qwen/Qwen2.5-Math-PRM-7B":
            self.tokenizer.truncation_side = "left"
        self.reward_model = reward_model

    def score(self, messages: Union[List[List[dict]], List[dict]], max_input_tokens: int = 4096, step_sep: str = "\n\n",
        ...
        if isinstance(messages[0], dict):
            ...
            messages = [messages]
        if self.reward_model == "Qwen/Qwen2.5-Math-PRM-7B":
            formatted_convs = []
            ...
            raise ValueError(f"Model {self.reward_model} is not supported")
