import math
from reward_model.base import AggregationMethod

from .base import AbstractProcessRewardModel
from reward_model import *


class LocalHallucinationScoreModel(AbstractProcessRewardModel):
    def __init__(
        self, model_types: list[str], model_name: str, reward_model: str
    ):
        self.models = [REWARD_MAP[model_type](
    ...
    def score(self, prompt: str, response_or_responses: str | list[str]) -> float:
        ...
        res = [math.prod(scores) for scores in zip(*all_res)]
...
class LocalVllmProcessRewardModel(AbstractProcessRewardModel):
    def __init__(
        self, model_name: str, aggregation_method: AggregationMethod
    ):
        from reward_hub.reward import VllmProcessRewardModel

        self.model = VllmProcessRewardModel(model_name=model_name, gpu_memory_utilization=0.2)
