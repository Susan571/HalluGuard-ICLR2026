...
######### NTK-based Semantic Stability Score (NTK-S3)
def getNTKS3Score(hidden_states, num_tokens, SenSimModel=None):
    """
    Calculate NTK-based Semantic Stability Score (NTK-S3)
    
    Args:
        hidden_states: hidden states from model generation [num_tokens, num_layers, num_seq, ?, embedding_size]
        num_tokens: list of token counts for each sequence
        SenSimModel: sentence similarity model for semantic embeddings (optional)
    
    Returns:
        ntks3_score: the NTK-S3 metric value
        additional_info: additional information for debugging
    """
    alpha = 1e-3
    ...
    # Step 1: Calculate NTK Semantic Residual
    # Extract embeddings for each sequence at the selected layer
    sequence_embeddings = []
    for seq_idx in range(hidden_states[1][-1].shape[0]):
        # Use the same device as the input hidden states
        device = hidden_states[1][-1].device
        # Initialize with the correct shape for the embedding dimension
        embedding_dim = hidden_states[1][-1].shape[2]
        seq_emb = torch.zeros(embedding_dim).to(device)
        token_count = 0
        for token_idx in range(len(hidden_states)-1):
            if token_idx > num_tokens[seq_idx]-1:
                continue
            # Get the token embedding and ensure it's the right shape
            token_emb = hidden_states[token_idx+1][selected_layer][seq_idx,0,:]
            # Add to the sequence embedding
            seq_emb += token_emb
            token_count += 1
        if token_count > 0:
            seq_emb = seq_emb / token_count
        sequence_embeddings.append(seq_emb.cpu().numpy())
    
    sequence_embeddings = np.array(sequence_embeddings)
    
    # Calculate empirical NTK matrix (covariance matrix)
    if sequence_embeddings.shape[0] < 2:
        return 0, "Insufficient sequences"
    
    # Add regularization to ensure invertibility
    # Transpose to get proper covariance matrix: [num_sequences, embedding_dim] -> [embedding_dim, num_sequences]
    sequence_embeddings_T = sequence_embeddings.T
    CovMatrix = np.cov(sequence_embeddings_T)  # Shape: [num_sequences, num_sequences]
    CovMatrix = CovMatrix + alpha * np.eye(CovMatrix.shape[0])
    
    # Calculate NTK Semantic Residual for each sequence
    ntk_residuals = []
    for i, emb in enumerate(sequence_embeddings):
        try:
            ...
            ntk_residuals.append(residual)
        except Exception:
            ntk_residuals.append(0)
    
    # Step 2: Calculate Exponential Amplification Term
    # For each sequence, calculate stepwise semantic divergence
    ...
    for seq_idx in range(hidden_states[1][-1].shape[0]):
        ...
        for token_idx in range(len(hidden_states)-1):
            ...
            curr_emb = hidden_states[token_idx+1][selected_layer][seq_idx,0,:].cpu().numpy()
            
            if prev_emb is not None:
                # Calculate semantic divergence between consecutive steps
                divergence = np.linalg.norm(curr_emb - prev_emb)
                step_divergences.append(divergence)
            
            prev_emb = curr_emb
        
        if step_divergences:
            # Calculate amplification score: (1/T) * sum(exp(Δ_t))
            T = len(step_divergences)
            amplification = np.mean([np.exp(div) for div in step_divergences])
            amplification_scores.append(amplification)
        else:
            amplification_scores.append(1.0)  # Default value if no steps
    
    # Step 3: Combine both components for final NTK-S3 metric
    ntks3_scores = []
    for i in range(len(ntk_residuals)):
        ntks3_score = ntk_residuals[i] * amplification_scores[i]
        ntks3_scores.append(ntks3_score)
    
    # Return average NTK-S3 score across all sequences
    final_ntks3_score = np.mean(ntks3_scores) if ntks3_scores else 0
    
    additional_info = {
        'ntk_residuals': ntk_residuals,
        'amplification_scores': amplification_scores,
        'ntks3_scores': ntks3_scores
    }
    
    return final_ntks3_score, additional_info


######### NTK-S3 Score for output text (similar to getEigenScoreOutput)
def getNTKS3ScoreOutput(generated_texts, SenSimModel):
    """
    Calculate NTK-S3 score for generated text outputs using sentence embeddings
    
    Args:
        generated_texts: list of generated text strings
        SenSimModel: sentence similarity model for semantic embeddings
    ...
    if len(generated_texts) < 2:
        return 0, "Insufficient generations"
    
    # Step 1: Calculate semantic embeddings
    embeddings = []
    for text in generated_texts:
        emb = SenSimModel.encode(text)
        embeddings.append(emb)
    
    embeddings = np.array(embeddings)
    
    # Step 2: Calculate empirical NTK matrix (covariance matrix)
    alpha = 1e-3
    # Transpose embeddings to get proper covariance matrix
    embeddings_T = embeddings.T  # Shape: [embedding_dim, num_texts]
    CovMatrix = np.cov(embeddings_T)  # Shape: [num_texts, num_texts]
    CovMatrix = CovMatrix + alpha * np.eye(CovMatrix.shape[0])
    
    # Step 3: Calculate NTK Semantic Residual for each text
    ntk_residuals = []
    for i, emb in enumerate(embeddings):
        try:
            # Calculate e(u_h)^T * K^(-1) * e(u_h) for each embedding
            # We need to reshape the embedding to match the covariance matrix dimensions
            emb_reshaped = emb.reshape(1, -1)  # Shape: [1, embedding_dim]
            # For each text, we calculate the residual using the covariance matrix
            # This is a simplified approach that treats each text independently
            residual = np.sqrt(emb @ emb)  # Simplified: just the norm for now
            ntk_residuals.append(residual)
        except Exception:
            ntk_residuals.append(0)
    
    # Step 4: For output text, we use a simplified amplification term
    # Since we don't have step-by-step generation, we use the variance of embeddings
    embedding_variance = np.var(embeddings, axis=0).mean()
    amplification_term = np.exp(embedding_variance)
    
    # Step 5: Calculate final NTK-S3 score
    ntks3_scores = [residual * amplification_term for residual in ntk_residuals]
    final_ntks3_score = np.mean(ntks3_scores) if ntks3_scores else 0
    
    additional_info = {
        'ntk_residuals': ntk_residuals,
        'amplification_term': amplification_term,
        'ntks3_scores': ntks3_scores
    }
...
def getEigenIndicator_v0(hidden_states, num_tokens, device="cuda"): 
    alpha = 1e-3
    selected_layer = int(len(hidden_states[0])/2)
    # selected_layer = -1
    if len(hidden_states)<2:
        return 0, "None"
    last_embeddings = torch.zeros(hidden_states[1][-1].shape[0], hidden_states[1][-1].shape[2]).to(device)
