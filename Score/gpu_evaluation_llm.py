...
def load_judge_model(device):
    """Load a model to serve as the judge for LLM-as-Judge evaluation"""
    try:
        print("Loading judge model (using GPT-2 for compatibility)...")
        judge_model, judge_tokenizer = load_real_model("gpt2", device)
        return judge_model, judge_tokenizer
    except Exception as e:
        print(f"Error loading judge model: {e}, using minimal model")
        # Fallback to minimal model if judge model fails to load
        return None, None

def evaluate_with_llm_judge(judge_model, judge_tokenizer, question, ground_truth, response, device):
    """Evaluate response quality using LLM-as-Judge approach"""
    if judge_model is None or judge_tokenizer is None:
        # Fallback to simple similarity if judge model is not available
        return calculate_simple_similarity(response, ground_truth)
    
    try:
        # Create evaluation prompt for the judge
        evaluation_prompt = f"""You are an expert evaluator. Please rate the quality of the following response to a question.

Question: {question}
Reference Answer: {ground_truth}
Generated Response: {response}

Please rate the response on a scale of 0.0 to 1.0, where:
- 0.0: Completely incorrect, irrelevant, or nonsensical
- 0.5: Partially correct but with significant issues
- 1.0: Completely correct, accurate, and well-articulated
...
Rating (0.0-1.0):"""
        ...
        inputs = judge_tokenizer(evaluation_prompt, return_tensors="pt", truncation=True, max_length=512)
        ...
        # Decode the judge's response
        judge_response = judge_tokenizer.decode(outputs[0][input_ids.shape[1]:], skip_special_tokens=True)
        ...
        try:
            ...
            import re
            numbers = re.findall(r'0\.\d+|1\.0|\d+\.\d+', judge_response)
            if numbers:
                rating = float(numbers[0])
                # Ensure rating is within [0, 1] range
                rating = max(0.0, min(1.0, rating))
                return rating
            else:
                # If no clear number found, use fallback
                return calculate_simple_similarity(response, ground_truth)
        except (ValueError, IndexError):
            # Fallback to simple similarity if parsing fails
            return calculate_simple_similarity(response, ground_truth)
...
def calculate_simple_similarity(response, ground_truth):
    """Fallback similarity calculation when LLM-as-Judge is not available"""
    ...
    if len(response_words) == 0 or len(truth_words) == 0:
        return 0.0
    
    intersection = len(response_words.intersection(truth_words))
    union = len(response_words.union(truth_words))
...
def calculate_real_semantic_entropy(responses):
    """Semantic entropy: cluster generations by lexical overlap, compute entropy."""
...
def calculate_real_race_score(model, tokenizer, responses):
    """RACE: representation agreement across generations."""
    model.eval()
    device = next(model.parameters()).device
    if not responses or len(responses) < 2:
        return 0.0
...
def calculate_real_lnpe_score(model, tokenizer, text):
    ...
    try:
        ...
        with torch.no_grad():
            ...
            layer_norms = [torch.norm(hs.float(), dim=-1).mean().item() for hs in outputs.hidden_states]
...
def calculate_real_factscore_score(model, tokenizer, text):
    """FActScore proxy: average token-level log-probability."""
    model.eval()
    device = next(model.parameters()).device
    try:
        inputs = tokenizer(text, return_tensors="pt", truncation=True, max_length=512)
...
def evaluate_method_corrected(model, tokenizer, questions, ground_truths, method_name, num_generations=3, judge_model=None, judge_tokenizer=None, device=None):
    """Evaluate a specific method with corrected responses using LLM-as-Judge"""
    ...
    for i, (question, ground_truth) in enumerate(zip(questions, ground_truths)):
        ...
        # Calculate correctness using LLM-as-Judge instead of ROUGE
        if judge_model is not None and judge_tokenizer is not None and device is not None:
            # Use LLM-as-Judge for evaluation
            correctness = max([evaluate_with_llm_judge(judge_model, judge_tokenizer, question, ground_truth, response, device) for response in responses])
        else:
            # Fallback to simple similarity if judge model is not available
            correctness = max([calculate_simple_similarity(response, ground_truth) for response in responses])
...
def run_gpu_evaluation():
    ...
    # Load judge model for LLM-as-Judge evaluation
    print("Loading judge model for LLM-as-Judge evaluation...")
    judge_model, judge_tokenizer = load_judge_model(device)
    
    # Methods to evaluate
    methods = ["INSIDE", "MIND", "Perplexity", "LN-Entropy", "Energy", "Semantic Entropy", "Lexical Similarity", "SelfCheckGPT", "RACE", "LNPE", "P(true)", "FActScore"]
