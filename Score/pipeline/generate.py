...
def get_generations(model_name:str, args, seed=1, old_sequences=None, max_num_gen_once=args.num_generations_per_prompt):
    ...
    # Download from Hugging Face instead of local path
    SenSimModel = SentenceTransformer('sentence-transformers/nli-roberta-large')
    bertscore = BERTScore(model_name_or_path="sentence-transformers/bert-base-nli-mean-tokens", device="cpu" if device == "cpu" else "cuda")
    ...
    for batch_idx, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        # Calculate NTK-S3 score for output text
        ntks3IndicatorOutput, ntks3InfoOutput = getNTKS3ScoreOutput(generated_texts, SenSimModel)
...
def main(overwrite=False, continue_from=None, parallel:int=None):
    if continue_from:
        ...
        old_sequences = []
        original_model_name = args.model  # Keep original name for model loading
        model_name = args.model
        if '/' in model_name:
            model_name = model_name.replace('/', '_')  # Processed name for file paths
    ...
    # Use original model name for model loading, processed name for display
